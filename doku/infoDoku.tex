...
\newpage %Wenn der Text geändert wird, muss dieses Kommando wieder entfernt werden
...
\begin{attribut}{aktualitaet}
	...
	\dbeschreibung{Die aktualitaet beschreibt vermutlich das Erstelldatum der Lokation. In \ref{bundAttributListe} ist dieses Datenfeld nicht dokumentiert. Anhand der aufgetretenen Werten wurde ermittelt, dass es sich dabei um ein Datum handeln muss. Der Wertebereich liegt grob zwischen 2008 und 2013. Die Vermutung, dass dieses Datenfeld das Erstelldatum oder Einpfegedatum der Lokation ist, stützt sich auf den Namen und den Wertebereich. Die Aktualitaet Klasse  \ref{class:Aktualitaet} im Programm stützt sich auf ein \glqq tm *zeit\grqq~struct, ist aber komfortabler zu nutzen.}
\end{attribut}

\begin{attribut}{linLocations}
	\drequired{nope}
	\dtype{vector\textless Linearlokation*\textgreater}
	\dbeschreibung{Jede Gebietslokation hat die Linearlokationen \ref{linearLokation}, die diese Gebietslokation als übergeordnet angegeben haben, in einem Vector gespeichert.}
\end{attribut}

\subsection{Linearlokation \label{linearLokation}}
Eine Linearlokation ist eine Straße. Dieser Typ von Lokation hat deutlich mehr Attribute als eine Gebietslokation. Jede Linearlokation hat eine Referenz auf die ihr übergeordnete Gebietslokation und beinhaltet selber Punktloaktionen. 
...
\begin{attribut}{secondName}
	...
	\dbeschreibung{Der secondName ist ein zusätzlicher Name einer Lokation. Ein Beispiel hierfür ist die Straße \glqq GDD12\grqq, \glqq Budapester Straße\grqq, \glqq Georgplatz\grqq, (roadNumber, roadName, secondName).}
\end{attribut}
...
\begin{attribut}{negativeOffset}
	...
	\dbeschreibung{Der negativeOffset ist grundsätzlich eine vorangehende oder nachfolgende Linear Location. Laut der Beschreibung des Bundes eine Vorgängerlokation bezogen auf die Erfassungsrichtung. Das Attribut ist als Pointer angelegt und erlaubt ein Iterieren durch die Lokationen. Für die Distanzberechnung wurde angenommen, dass es zwischen den Offset eine direkte Verbindung gibt. Zwischen negativ und positiv wurde bei der Suche der Nachfolger kein Unterschied gemacht.}
\end{attribut}
...
\begin{attribut}{interruptsRoad}
	...
	\dbeschreibung{Zeigt auf die nächste Punktlokation nach einer Unterbrechung im Straßenverlauf, wie z.b. durch den intersectioncode. Durch Versuche hat sich ergeben, dass dieses Attribut nicht zur Routenführung zu gebrauchen ist.}
\end{attribut}
...
\begin{attribut}{inPositive}
	...
	\dbeschreibung{inPositive beschreibt, ob die Location in Erfassungsrichtung zugänglich ist. }
	\dbelegung{0 = nein = false, 1 = ja = true}
\end{attribut}
...
Eine Punktlokation ist eine Lokation, die weder ein Gebiet noch eine Straße ist. Punktlokationen haben eine übergeordnete Linearlokation.
