...
\todo{bsp: c++ 11 für auto Iteratoren}
...
%Die Berechnung der Distanzen läuft nach dem Dijkstra Algorithmus. Für diese Berechnung liegt eine PriorityQueue zugrunde, die auch in der gleichnamigen Klasse implementiert ist. Kurz beschrieben, von dem Element, was aktuell die kürzeste Distanz zum Startknoten hat und noch nicht besucht wurde, werden die Nachfolger in die Priority Queue eingetragen. Nachdem die Nachfolger eingetragen wurden, wird das Flag \textit{besucht} dieses Elements true gesetzt. Beim Eintragen der Nachfolger wird geprüft, ob sie bereits in der Queue enthalten sind. Ist das der Fall, wird geprüft, ob die neue Distanz eine Verbesserung zur Bestehenden darstellt.  Bei Bedarf wird die Distanz verändert und das Flag \textit{besucht} false gesetzt. Dadurch wird gewährleistet, dass die Nachfolger der soeben verbesserten Elemente auch wieder relaxiert werden. 
